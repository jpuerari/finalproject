...
function BucketList() {
  ...
  return (
    <>
      ...
      <Container fluid>
        <h2 className='search'>
          {userData.savedPlaces.length || userData.savedCities.length
            ? `Viewing ${userData.savedPlaces.length + userData.savedCities.length} saved ${
                userData.savedPlaces.length + userData.savedCities.length === 1 ? 'Place' : 'Places'
              }:`
